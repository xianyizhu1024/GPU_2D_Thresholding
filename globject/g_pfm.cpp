...
void GPfm::load(int width, int height, const unsigned int *pr, const unsigned int *pg, const unsigned int *pb)
{
	load(width, height);

	FLOAT3 *tm = fm;
	const unsigned int *tr = pr;
	const unsigned int *tg = pg;
	const unsigned int *tb = pb;
...
FLOAT4 GPf4::lookup_nearest(float sx, float sy) const
{
	int x0, y0;
	x0 = (int)G_CLAMP(sx, 0, w - 1);
	y0 = (int)G_CLAMP(sy, 0, h - 1);
...
void GPf4::load( int width, int height, const FLOAT3 *rgb, const float *a )
{
  load( width, height );
...
void GPf4::getblk( GPf4 &blk, int x, int y, int blkw, int blkh ) const
{
  if( blk.w != blkw || blk.h != blkh )
    blk.load( blkw,blkh );
  ...
  for( j=0; j<blkh; j++ )
    memcpy( blk.pm[j], &pm[y+j][x], blkw * sizeof(FLOAT4) );
}
...
float GPf1::lookup_linear( float sx, float sy ) const
{
  float qx, qy;
  float rx, ry;
    qx = floorf(sx-.5f)+.5f;
  ...
  float val;
  val = g_lerp(
      g_lerp( pm[y0][x0], pm[y0][x1], rx ),
      g_lerp( pm[y1][x0], pm[y1][x1], rx ), ry
    );

  return val;
}

void GPf1::mul( float a )
{
  int i;
  for( i=0; i<w*h; i++ )
    fm[i] *= a;
...
void GPng::load( const char *spath, int num_channel )
{
  IHDR ihdr;
  FILE *f0 = fopen( spath, "rb" );
  if( f0==NULL )
  ...
  }else
  {
    ...
    do
    {
      ...
      switch( chk_id )
      {
        case 'IHDR':
          ...
          if( ihdr.Colour!=2 && ihdr.Colour!=3 && ihdr.Colour!=6 )
          {
          ...
          if( ihdr.Interlace==1 )
          {
            if( ihdr.Colour==6 )
            {
              printf("[Error] GPng::load(), Color Type not supported in interlaced mode.  Colour=%i\n", ihdr.Colour);
              exit(-1);
            }
            if( ihdr.Colour==3 && ihdr.Bit!=8 )
            {
              printf("[Error] GPng::load(), Color Depth not supported in interlaced mode.  Colour Depth=%i\n", ihdr.Bit);
              exit(-1);
            }
          }
          if(ihdr.Colour==2)
          ...
          if(ihdr.Colour==6)
            nb=4;
          n_buf = (ihdr.Width*nb+1)*ihdr.Height*sizeof(unsigned char);
          src = (unsigned char*)malloc(n_buf);
        break;
        ...
        case 'tRNS':
        {
          tRNS = (unsigned char*)malloc( chk_len );
          fread( tRNS, sizeof(unsigned char), chk_len, f0 );
        }
        break;
    
        case 'IDAT':
        {
          if( n_src+chk_len > n_buf )
          {
            n_buf += chk_len*16;
            src = (unsigned char*)realloc( src, n_buf );
          }
    ...
    int type;
    int n_des;
    unsigned char *des=0, *trow;
    unsigned char *drow=0, *erow=0;

    if( ihdr.Interlace==0 )
    {
      drow = (unsigned char*)malloc( ihdr.Width*nb * sizeof(unsigned char) );
      erow = (unsigned char*)malloc( ihdr.Width*nb * sizeof(unsigned char) );
      memset( drow,       0, ihdr.Width*nb * sizeof(unsigned char) );
      n_des = ( (ihdr.Width*nb*ihdr.Bit+7)/8 +1)* ihdr.Height;
      des = (unsigned char*)malloc( n_des*sizeof(unsigned char) );
      decompress( src, n_src, des, n_des );
      
      if( ihdr.Colour==2 || ihdr.Colour==6 )
      {
        load( ihdr.Width, ihdr.Height, num_channel==0 ? nb : num_channel );
        for( j=0; j<h; j++ )
        {
          trow = &des[(w*nb+1)*j];
          type = trow[0];
          memcpy( erow, &trow[1], w*nb * sizeof(unsigned char) );
          filter( erow, drow, type, nb );
          drawrow( pm[j], nc, erow, nb, w );
          g_swap(drow,erow);
        }
      }
      ...
      if( ihdr.Colour==3 )
      {
        if( num_channel )
          ...
          load( ihdr.Width, ihdr.Height, tRNS ? 4 : 3 );
        if( ihdr.Bit==1 || ihdr.Bit==2 || ihdr.Bit==4 )
        {
          for( j=0; j<h; j++ )
          {
            unsigned int bmask;
            ...
            trow = &des[((w+(7/ihdr.Bit))/(8/ihdr.Bit)+1)*j+1];
            bmask = (1<<ihdr.Bit)-1;
            for( i=0; i<w; i++ )
            {
              ppos = i/(8/ihdr.Bit);
              bsht = ((7/ihdr.Bit)-i%(8/ihdr.Bit))*ihdr.Bit;
              memcpy( &pm[j][nc*i+0], &palette[ (trow[ppos]&(bmask<<bsht)) >> bsht ], 3 );
            }
            if(nc==4)
              for( i=0; i<w; i++ )
                pm[j][nc*i+3] = 255;
        ...
        }else
        {
          ...
          {
            trow = &des[(w+1)*j+1];
            drawrow( pm[j], nc, trow, palette, tRNS, w );
          }
        }
      }
    }
        ...
        for( k=0, trow=des; k<7; k++ )
          for( j=ps[k][3]; j<h; j+=ps[k][1] )
            for( i=ps[k][2], trow++; i<w; i+=ps[k][0], trow++ )
              memcpy( &pm[j][nc*i+0], &palette[ *trow ], 3 );
            if(nc==4)
            for( i=0; i<w; i++ )
              pm[j][nc*i+3] = 255;
      }
...
void GPng::drawrow( unsigned char *des, int db, const unsigned char *src, int sb, int w )
{
  int i, b;
  float s;
  unsigned char *td;
  ...
  {
    for( i=0, td=des, ts=src; i<w; i++, td+=db, ts+=sb )
    {
      s = float(ts[3])/255;
      for( b=0; b<db; b++ )
        td[b] = ftoi((float(ts[b])/255)*s,255);
    }
  }else if( db==4 && sb==3 )
  {
    for( i=0, td=des, ts=src; i<w; i++, td+=db, ts+=sb )
...
void GPng::drawrow( 
  unsigned char *des, int db, const unsigned char *src, 
  const BYTE3 *palette, const unsigned char *tRNS, int w )
{
  int i;
  float s;
  ...
  if( db==4 && tRNS )
  {
    for( i=0; i<w; i++ )
    {
      memcpy( &des[db*i+0], &palette[ src[i] ], 3 );
      memcpy( &des[db*i+3],    &tRNS[ src[i] ], 1 );
    }
  }else if( db==3 && !tRNS )
  {
    for( i=0; i<w; i++ )
  ...
  }else if( db==3 && tRNS )
  {
    for( i=0; i<w; i++ )
    {
      s = float(tRNS[ src[i] ])/255;
      des[db*i+0] = ftoi((float(palette[src[i]].x)/255)*s,255);
      des[db*i+1] = ftoi((float(palette[src[i]].y)/255)*s,255);
      des[db*i+2] = ftoi((float(palette[src[i]].z)/255)*s,255);
    }
  }
}

void GPng::load( int width, int height, int num_channel )
{
  SAFE_FREE(bm);
  SAFE_FREE(pm);
  ...
  h = height;
  nc = num_channel;
  bm = (unsigned char*)malloc( w * h * nc );
  memset( bm, 0, w * h * nc );
  pm = (unsigned char**) malloc( h * sizeof(unsigned char*) );
  int j;
  for( j = 0; j<h; j++ )
    pm[j] = &bm[j*w*nc];
}
...
{
  load( width, height );
  const float *ft = (float*)fm;
  unsigned char *bt = bm;
  int i,j,b;
  for( j=0; j<h; j++ )
  for( i=0; i<w; i++ )
  for( b=0; b<nc; b++, bt++, ft++ )
    *bt = (GBYTE)ftoi(*ft,255);
}
...
void GPng::filter( void *curline0, const void *prvline0, int type, int nb )
{
  int i, b;
  unsigned char *src, *des;
  unsigned char *ps, *pd;
  src = (unsigned char *)prvline0;
  des = (unsigned char *)curline0;

  switch(type)
  {
    case 1:
      for( i=1, pd=&des[nb]; i<w; i++, pd+=nb )
      for( b=0; b<nb; b++ )
        pd[b] += pd[b-nb];
      break;
    case 2:
      for( i=0, ps=src, pd=des; i<w; i++, pd+=nb, ps+=nb )
      for( b=0; b<nb; b++ )
        pd[b] += ps[b];
      break;
    case 3:
      for( b=0; b<nb; b++ )
        des[b] += src[b]/2;
      for( i=1, ps=&src[nb], pd=&des[nb]; i<w; i++, pd+=nb, ps+=nb )
      for( b=0; b<nb; b++ )
        pd[b] += ( ps[b] + pd[b-nb] )/2;
      break;
    case 4:
      for( b=0; b<nb; b++ )
        des[b] += Paeth( 0,src[b],0 );
      for( i=1, ps=&src[nb], pd=&des[nb]; i<w; i++, pd+=nb, ps+=nb )
      for( b=0; b<nb; b++ )
        pd[b] += Paeth(pd[b-nb],ps[b],ps[b-nb]);
      break;
  }
}
...
{
  if( blk.w != blkw || blk.h != blkh )
    blk.load( blkw,blkh,nc );

  int j;

  for( j=0; j<blkh; j++ )
    memcpy( blk.pm[j], &pm[y+j][x], blkw * nc );
}
...
void GPng::flip_vertical()
{
  ...
  int j;
  for( j=0; j<h; j++ )
    memcpy( tmp.pm[j], pm[h-j-1], w*nc );
    memcpy( bm, tmp.bm, w*h*nc );
}
...
#ifdef _DEBUG
#pragma comment (lib, "../../libs/zlib/lib/zlibd.lib")  // Link with zlib in Debug Mode
#else
#pragma comment (lib, "../../libs/zlib/lib/zlib.lib")  // Link with zlib in Release Mode
#endif
...
void GPng::save( const char *spath ) const
{
  ...
  unsigned int n_cdata;  
    cdata = (unsigned char*) malloc( 2*(w*nc+1) * h * sizeof(unsigned char) );
    {
      unsigned char *fdata, *rdata;
      unsigned int n_fdata;
        fdata = (unsigned char*) malloc( (w*nc+1) * h * sizeof(unsigned char) );
        n_fdata = (w*nc+1) * h;
        n_cdata = n_fdata;
        for( j=0; j<h; j++ )
        {
          rdata = &fdata[j*(w*nc+1)];
          rdata[0] = filtertype;

          // implementation of filter type 0
          memcpy( &rdata[1], pm[j], w * nc );
        }    
...
extern "C" {  
#include "../../libs/JpegLib/include/jpeglib.h" 
}
#ifdef _DEBUG
#pragma comment (lib, "../../libs/JpegLib/lib/JpegLibd.lib")  // Link with JpegLib in Debug Mode
#else
#pragma comment (lib, "../../libs/JpegLib/lib/JpegLib.lib")  // Link with JpegLib in Release Mode
#endif
...
void GPim::load( const GPfm &img, int number_of_code, int max_cycle )
{
  ...
  GPf1 src;
    ...
    lbg_training( src, max_cycle, codebook );

  load( img.w, img.h, number_of_code );

  coltbl.load( n_code, 1, (FLOAT3*)codebook.fm );
...
void GPim::lbg_training( const GPf1 &src, int max_cycle, GPf1 &codebook )
{
...
void GPdm::load( int width, int height )
{
  SAFE_FREE( fm );
  SAFE_FREE( pm );
  ...
  w = width;
  h = height;
  fm = (DOUBLE3*) malloc( w * h * sizeof(DOUBLE3) );

  if(fm==0)
  {
    printf( "%i %i %i\n", w * h * sizeof(DOUBLE3), w, h );
  }

  memset( fm, 0, w*h*sizeof(DOUBLE3) );

  pm = (DOUBLE3**) malloc( h * sizeof(DOUBLE3*) );
  int j;
  for( j=0; j<h; j++ )
    pm[j] = &fm[j*w];
}

void GPdm::load( const char *spath )
{
  GPfm src;
  src.load( spath );
  load( src.w, src.h, src.fm );
}

void GPdm::save( const char *spath )
{
  GPfm src;
  int i;
  ...
  for( i=0; i<w*h; i++ )
  {
    src.fm[i].x = float(fm[i].x);
    src.fm[i].y = float(fm[i].y);
    src.fm[i].z = float(fm[i].z);
  }

  src.save( spath );
}

void GPdm::load( int width, int height, const FLOAT3 *src )
{
  int i;
  load(width,height);
  for( i=0; i<w*h; i++ )
