...
class GPfm
{
  public:
	...
	void load( int width, int height, const unsigned int *pr, const unsigned int *pg, const unsigned int *pb);
...
class GPf4
{
  public:
    ...
    void load( int width, int height, const FLOAT3 *rgb, const float *a );
    void load( int width, int height, const FLOAT4 *rgba );
    void load( int width, int height, const FLOAT3 *rgb );
    void load( int width, int height, FLOAT4 a );
    void load( int width, int height );
    ...
    void getblk( GPf4 &blk, int x, int y, int blkw, int blkh ) const;
	...
	FLOAT4 lookup_nearest(float x, float y) const;
...
class GPf1
{
  public:
    ...
    void scale( int width, int height );
    float lookup_linear( float sx, float sy ) const;
...
class GPng
{
  public:
    ...
    void load( int width, int height, const FLOAT3 *fm );
    void load( int width, int height, const BYTE3 *bm );
    void load( int width, int height, int num_channel=3 );
    void load( const char *spath, int num_channel=3 );
    void save( const char *spath ) const;
  ...
  private:
    ...
    static void drawrow( unsigned char *des, int db, const unsigned char *src, int sb, int w );
    static void drawrow( 
  unsigned char *des, int db, const unsigned char *src, 
  const BYTE3 *palette, const unsigned char *tRNS, int w );

    void filter( void *curline, const void *prvline, int type, int nb );
...
class GPim
{
  ...
  private:
    ...
    static void lbg_training( const GPf1 &src, int max_cycle, GPf1 &codebook );
...
class DOUBLE3
{
  public:
    ...
    DOUBLE3():x(0),y(0),z(0){};
    DOUBLE3& operator+=( const DOUBLE3 &a ){ x+=a.x; y+=a.y; z+=a.z; return *this; }
    friend DOUBLE3 operator*( double a, const DOUBLE3 &b ){ return DOUBLE3( a*b.x, a*b.y, a*b.z ); }
};
